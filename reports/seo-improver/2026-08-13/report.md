...
**Sitemap submitted to Google Search Console by Brett the same day, after the deploy** — so the three new URLs got discovery help immediately rather than waiting on an organic crawl.

**What this changes for next week's run:** the 2026-08-20 measurement is the first clean read on week 3's three guides and all of week 4's schema work. The three keywords marked as 404-at-measurement in `rankings.csv` get a real baseline for the first time — if the week-2 pattern holds (~7 days from live to ranking), they should show movement next run. Treat next week's numbers as the actual test of the guide-page strategy at 6 pages, now with a GSC submit in the mix that week 2 did not have.
...
- ~~**Submit the updated sitemap to Google Search Console.**~~ ✅ **Done — Brett submitted it on 2026-08-13, after the deploy.** This clears the longest-standing backlog item in the loop and, combined with the same-day deploy, means the three new guides get discovery help that week 2's guides never had. Week 2's guides took ~7 days to rank *without* a manual submit; if next week's run shows the new guides ranking faster than that, the submit is the most likely reason. Still no GSC *access* from this loop, so indexation status remains something only a human can read.
